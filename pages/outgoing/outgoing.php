  ...
  //Include connection file 
   ...
   if(session_id() == '') {
    ...
    if (!$_SESSION['user']) {
      ...
      $sql_login = 'SELECT DISTINCT * FROM outgoing INNER JOIN tariff ON outgoing.outgoing_tariff = tariff.tariff_id INNER JOIN measure ON measure.measure_id = outgoing.outgoing_measure INNER JOIN pednum ON pednum.pednum_id = outgoing.outgoing_pednum ORDER BY pednum.pednum_desc ' ;
...
<body class="">
      ...
      <div class="content-header content-header-lg">
        <div class="row">
          <div class="col-md-12 text-right" id="ConfButton">
            <div >
              <button type="button" type="submit" class="btn btn-reddark" onclick="toCsv()"><i class="fas fa-edit"></i> &nbsp;Importar Salidas</button>
            <button type="button" class="btn btn-reddark" onclick="newEntry()"><i class="fas fa-plus"></i> &nbsp;Nueva Salida</button>
            </div> 
          </div>
          </div>
          <!-- Modal -->
              ...
              <table class="table table-sm table-hover table-dark ">
                ...
                <tbody id="sTProduct">
                  ...
                  while ($row = mysqli_fetch_array($result_login )){
                      ...
                      echo '<tr class="" data-href="entryoutgoing.php?outgoing_name='.$pednum.'">';
  ...
  <!-- Custom Script -->
    ...
    $.ajax({
               ...
               data: {
                  btn2: 'Cancelar',
                  func: "document.location.href='../outgoing/outgoing.php'"
               },
    ...
    function insertOutry(){
     
             $.ajax({
               type: "POST",
               url: "../queries/insertOutry.php",
               data: {
                  pednum:$('#npedi').val(),
                  pedkey:'RT',
                  peddate:$('#date').val(),
               ...
               success: function (data) {
                $('#product_updated').html(data);
               // document.location.href='../outgoing/outgoing.php';
               }
            });
    }
    function reportDate(){
      if(!$('#startDate').val() || !$('#endDate').val()){
       ...
       $.ajax({
          type: "POST",
          url: "../queries/outgoingReport.php",
          data: {
